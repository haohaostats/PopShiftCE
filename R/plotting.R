...
#' - `pC`: conditional critical value function `c(t1)` (often near-flat in the
#'   baseline setting and therefore typically more useful as a supplementary
#'   diagnostic);
#' - `pD`: marginal density of `T1` under `H0`.
#'
#' If `combine = TRUE`, a patchwork composite (2x2 panel) of the CE calibration
#' diagnostics is returned (A/B/C/D in the order above).
#'
...
plot_ce_mapping <- function(H0_dt, lookup, combine = FALSE, bins = 120) {
  ...
  # B: Conditional error function e(t1)
  tgrid <- lookup$z1_grid
  pB <- gg$ggplot(
    data.frame(t1 = tgrid, e = lookup$e_fun(tgrid)),
    gg$aes(t1, e)
  ) +
    gg$geom_line() +
    gg$geom_vline(xintercept = lookup$b_ref, linetype = 2) +
  ...
  # C (supplementary-style diagnostic): c(t1)
  pC <- gg$ggplot(
    data.frame(t1 = tgrid, c = lookup$c_fun(tgrid)),
    gg$aes(t1, c)
  ) +
    gg$geom_line() +
    gg$geom_vline(xintercept = lookup$b_ref, linetype = 2) +
  ...
  # D: Marginal density of T1 under H0
...
#' Four-panel diagnostic panel for the main workflow
#'
#' Combines CE calibration diagnostics from a null simulation with the decision
#' geometry under an alternative simulation, following the paper-style layout:
#'
#' - Panel A: joint null cloud `(T1, S_final)`
#' - Panel B: conditional error function `e(t1)`
#' - Panel C: marginal density of `T1` under `H0`
#' - Panel D: decision geometry under an alternative scenario
#'
#' (The conditional critical value curve `c(t1)` is available via
#' [plot_ce_mapping()] and is often best treated as a supplementary diagnostic in
...
plot_diagnostic_panel <- function(h0_results, alt_results, lookup) {
  ...
  # Updated paper-style layout:
  # A = joint null cloud, B = e(t1), C = marginal T1 density, D = decision geometry
  (ce_plots$pA | ce_plots$pB) / (ce_plots$pD | p_decision) +
